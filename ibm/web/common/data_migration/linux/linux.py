...
from ..consts import QEMU_CUSTOM_INSTALLATION
from ..operating_system import BaseOS
...
class Linux(BaseOS):
    ...
    def bash_installation_string(self):
        ...
        com_string = self.INSTALL_PACKAGE_COMM.format(
            package_manager=self.PACKAGE_MANAGER, package_name=packages
        )
        if self.UPDATE:
            update_string = self.UPDATE_COMM.format(
                package_manager=self.PACKAGE_MANAGER
            )
            com_string = """{update_string} \n{com_string}""".format(
                update_string=update_string, com_string=com_string
            )
        if self.EXPORT_NON_INTERACTIVE:
            com_string = "{export_variable} \n{com_string}".format(
                export_variable=self.EXPORT_NON_INTERACTIVE_COMM, com_string=com_string
            )
        if self.qemu_custom_installation:
            com_string = "{com_string} \n {qemu_custom_installation}".format(
                com_string=com_string,
                qemu_custom_installation=QEMU_CUSTOM_INSTALLATION)
        return com_string

    @property
    def packages(self):
        packages = self.qemu_package
