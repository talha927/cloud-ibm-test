...
PACKAGES_INSTALLATION_FIX = """
ip route flush 161.26.0.0/16
rm /etc/sysconfig/network-scripts/route-eth0
/var/lib/cloud/instance/scripts/vendor/part-004
"""

LINUX_SVM_REQUIREMENTS = """#!/bin/bash
INSTANCE_NAME="{INSTANCE_NAME}"
LIST_OF_COS_IMAGES={LIST_OF_COS_IMAGES}
...
function download_cos_image() {
  ...
  while ((i < 10)); do
    ...
    echo "$COS_OBJECT_FILE started DOWNLOADING at: $(date)" >>$REPORT_FILE
    generate_ibm_iam_token
    wget -c -o download_status.log https://s3.direct."$REGION".cloud-object-storage.appdomain.cloud/"$BUCKET"/"$COS_OBJECT_FILE" --header "Authorization: $iam_token"
    if [ $? -eq 0 ]; then
      img_downloaded=true
      break
    else
  ...
  if $img_downloaded; then
    echo "$COS_OBJECT_FILE took $total_download_time seconds for DOWNLOADING.." >>$REPORT_FILE
    img_info=$(qemu-img info $COS_OBJECT_FILE | grep -Po 'virtual size: \K.*G' || qemu-img info $COS_OBJECT_FILE | grep -Po 'virtual size: \K.*T')
  fi
}
...
function extract_image_to_disk() {
  img_extracted=false
  cd $download_path || return
  echo "$COS_OBJECT_FILE started EXTRACTING on /dev/$disk at: $(date)" >>$REPORT_FILE
  extraction_start_time=$(date +%s)
  image_format="${COS_OBJECT_FILE##*.}"
  if [ "$image_format" == "vhd" ]; then
      image_format="vpc";
  fi
  extract_status=$(qemu-img convert -f $image_format "$COS_OBJECT_FILE" /dev/"$disk" 2>&1)
  extraction_end_time=$(date +%s)
  total_extraction_time=$(($extraction_end_time - $extraction_start_time))
  if [ -z "$extract_status" ]; then
    img_extracted=true
    cd $download_path && rm -rf $COS_OBJECT_FILE
    echo "$COS_OBJECT_FILE took $total_extraction_time seconds for EXTRACTION.." >>$REPORT_FILE
  fi
}
...
function report_vpc_plus() {
    ...
    "status": "$status",
    "resources": [],
    "start_time": "$VOLUME_MIG_START_TIME",
    "end_time": "$VOLUME_MIG_END_TIME",
    "duration": "$TOTAL_VOLUME_MIG_TIME",
    "action": "",
    "message": "$trace"
  }
END
)
curl -X PUT $WEB_HOOK_URI -H "Accept: application/json" -H "Content-Type: application/json" --data-binary @- <<DATA
$response
DATA

  if [ "$status" = "FAILED" ] || [ "$status" = "SUCCESS" ]; then
    echo "--------------Volume Migration for $COS_OBJECT_FILE took $TOTAL_VOLUME_MIG_TIME Seconds-------------" >>$REPORT_FILE
    exit
  fi
}
...
function from_image_to_disk() {
  #set download_path and COS_OBJECT_FILE
  download_cos_image
  if ! $img_downloaded; then
    echo "$COS_OBJECT_FILE Failed to DOWNLOAD..!" >>$REPORT_FILE
    DISK_STATUS="FAILED"
    return
  fi
  ...
  if ! $img_extracted; then
    echo "$COS_OBJECT_FILE Failed to EXTRACT..! Reasons: $extract_status" >>$REPORT_FILE
    DISK_STATUS="FAILED"
    return
  fi
  mount_disk
...
for pkg in "${PACKAGES[@]}"; do
  installation
  if [ $? != 0 ]; then
    trace="Unable to install $pkg,Volume Migration Failed, This is an issue in primary Image...!"
...
if $mounted; then
  ...
  trace="Reasons: $create_stats, $mount_stats" >>$REPORT_FILE
  echo $trace >>$REPORT_FILE
...
for COS_OBJECT_FILE in "${LIST_OF_COS_IMAGES[@]}"; do
  VOLUME_MIG_START_TIME=$(date +%s)
  echo "########################################################################" >>$REPORT_FILE
  echo "--------------Volume Migration for $COS_OBJECT_FILE started --------------------" >>$REPORT_FILE
  from_image_to_disk
...
WINDOWS_SVM_REQUIREMENTS = """
$global:region = "{REGION}"
$global:region_id = "{REGION_ID}"
...
$global:cloud_id = "{CLOUD_ID}"
$global:vhds = @({LIST_OF_COS_IMAGES})
"""
...
workflow windows-svm-workflow{{ InlineScript{{
        ...
        $global:access_token = ""
        $global:report = @{{ "status" = "IN_PROGRESS"; "message" = ""; "start_time" = ""; "end_time" = ""; "duration" = ""; "resources" = @(); "action" = ""; }}
        ...
        function Send-Response {{
            Set-Log -message $global:report.action
            $header = @{{ "Content-Type" = "application/json" }}
            $res = Retry-Command -ScriptBlock {{
                wget -Method PUT $global:web_hook_url -H $header -Body ($global:report | ConvertTo-Json) -UseBasicParsing -ErrorAction stop
            }}
            return $res
        }}
        ...
        function Get-LargeDisk {{ $disk = Get-Disk | Where-Object {{ $_.PartitionStyle -eq 'RAW' -and $_.Size -ge 1000000000 -and ($_.SerialNumber -split '-').Count -gt 0 -and ($_.SerialNumber -split '-')[0].Length -eq 4 }} | Sort-Object -Property size -Descending | Select-Object -First 1
        ...
        function Download-Vhd {{ param($vhd, $drive_letter)
            ...
            Send-Response
            $url = "https://s3.$global:region.cloud-object-storage.appdomain.cloud/$global:bucket/$vhd"
            $header = @{{ "Authorization" = "Bearer $global:access_token" }}
            $output = "${{drive_letter}}:\$vhd"
        ...
        function Migrator {{ param($partition_style, $disk_number, $base_drive_letter, $resource_name, $Actual_mounted_folder_path, $index)
            ...
            $mounted_vhd_drive = GET-DISKIMAGE "${{base_drive_letter}}:\$resource_name" | GET-DISK | GET-PARTITION -ErrorAction stop
        ...
        function Remove-resource {{ param($vhd, $physical_drive_letter)
            ...
            Send-Response
            DISMOUNT-DISKIMAGE "${{physical_drive_letter}}:\$vhd" -ErrorAction stop
            Remove-Item -Path "${{physical_drive_letter}}:\$vhd" -recurse -ErrorAction stop
        ...
        function Lets-Migrate {{ $base_disk = Get-LargeDisk
            ...
            foreach ($resource in $global:report.resources) {{ $resource_name = $resource.name
                try {{  $start = (Get-Date)
                    ...
                    MOUNT-DISKIMAGE "${{base_drive_letter}}:\$resource_name" -ErrorAction stop
                    $downloaded_vhd = GET-DISKIMAGE "${{base_drive_letter}}:\$resource_name" -ErrorAction stop
