from ibm.common.clients.softlayer_clients.instances.consts import VENDOR_DICTIONARY
from ibm.common.consts import CREATION_PENDING
from ibm.common.utils import get_network, is_private, is_private_ip, remove_duplicates, transform_ibm_name
from ibm.models import (IBMIKEPolicy, IBMImage, IBMInstance, IBMInstanceProfile, IBMIPSecPolicy, IBMListener,
                        IBMLoadBalancer, IBMNetworkAcl, IBMNetworkAclRule, IBMNetworkInterface, IBMOperatingSystem,
                        IBMPool, IBMPoolHealthMonitor, IBMPoolMember, IBMSecurityGroup, IBMSecurityGroupRule, IBMSshKey,
                        IBMSubnet, IBMVolume, IBMVolumeAttachment, IBMVolumeProfile, IBMVpnConnection, IBMVpnGateway)
...
class SoftLayerInstance(object):
    ...
    STATUS_KEY = "status"
    MONITORING_INFO_KEY = "monitoring_info"

    def __init__(self, name, image=None, ssh_keys=None, instance_profile=None, network_interfaces=None,
                 volumes=None, instance_id=None, data_center=None, instance_type=None, dedicated_host=None,
                 placement_group=None, auto_scale_group=None, metric_tracking_object_id=None,
                 network_attached_storages=None, status=None, monitoring_info=None):
        self.name = name
        self.image = image
        self.ssh_keys = ssh_keys or list()
        self.instance_profile = instance_profile
        self.network_interfaces = network_interfaces or list()
        self.volume_attachments = volumes or list()
        ...
        self.metric_tracking_object_id = metric_tracking_object_id
        self.status = status
    ...
    def from_softlayer_json(cls, instance_json):
        return cls(
            ...
            dedicated_host=instance_json["dedicatedHost"] if instance_json.get('dedicatedHost') else None,
            placement_group=instance_json["placementGroup"] if instance_json.get("placementGroup") else None,
            metric_tracking_object_id=instance_json.get('metricTrackingObjectId'),
            status=instance_json["status"].get("keyName")
        )
    ...
    def to_json(self):
        resp = {
            ...
            self.VOLUME_KEY: [v.to_json() for v in self.volume_attachments],
            ...
            self.MONITORING_INFO_KEY: self.monitoring_info.to_json() if self.monitoring_info else None,
        }

        if self.dedicated_host:
            resp[self.DEDICATED_HOST_KEY] = {
                self.ID_KEY: self.dedicated_host["id"],
                self.NAME_KEY: self.dedicated_host["name"],
            }
        if self.placement_group:
            resp[self.PLACEMENT_GROUP_KEY] = {
                self.ID_KEY: self.placement_group["id"],
                self.NAME_KEY: self.placement_group["name"],
            }

        return resp

    def to_ibm(self):
        ibm_instance = IBMInstance(name=self.name, status=CREATION_PENDING)
        ...
        ibm_instance.ssh_keys = self.ssh_keys
        for volume_attachment in self.volume_attachments:
            ibm_volume_attachment = volume_attachment.to_ibm()
            if ibm_volume_attachment.type_ in ["data", "boot"]:
    ...
    @classmethod
    def from_softlayer_json_body(cls, instance_json, image_guid):
        return {
            "cpus": instance_json['maxCpu'],
            "memory": instance_json['maxMemory'],
            "hourly": instance_json['hourlyBillingFlag'],
            "hostname": "v" + instance_json['hostname'][-7:] + "backup",
            "domain": instance_json['domain'],
            "datacenter": instance_json["datacenter"]["name"],
            "image_id": image_guid,
            "local_disk": False,
            "userdata": InstanceMigrationConsts.IBM_GEN2_PRE_REQ_FOR_WINDOWS_POWERSHELL_SCRIPT,
            "dedicated": instance_json.get('dedicatedAccountHostOnlyFlag'),
            "private": instance_json.get('privateNetworkOnlyFlag'),
            "tags": "DO-NOT-DELETE, TEMPORARY, WINDOWS-BACKUP, AUTO-DELETE",
            "ssh_keys": [sshkey.id for sshkey in instance_json['sshKeys']] if instance_json.get('sshKeys') else [],
            "metric_tracking_object_id": instance_json.get('metricTrackingObjectId')
        }
...
class SoftLayerImage(object):
    ...
    def from_softlayer_json(cls, operating_system):
        return cls(name=operating_system.get("longDescription"), os_name=operating_system.get("longDescription"),
                   ...
                   os_vendor=VENDOR_DICTIONARY.get(operating_system["manufacturer".lower()]))
...
class SoftLayerInstanceProfile(object):
    ...
    def __init__(self, name, family, max_cpu, max_memory):
        self.name = name
        self.family = family
    ...
    def to_json(self):
        return {
            ...
            self.MAX_CPU_KEY: self.max_cpu,
            self.MAX_MEMORY_KEY: self.max_memory
        }

    def to_ibm(self):
        return IBMInstanceProfile(self.name, self.family, self.max_cpu, self.max_memory)
...
class SoftLayerVolume(object):
    ...
    def __init__(self, name, capacity, profile, iops, delete_volume, is_boot_volume=None, volume_index=0):
        self.name = name
    ...
    def to_ibm(self):
        ibm_volume_attachment = IBMVolumeAttachment(
            ...
            delete_volume_on_instance_delete=self.delete_volume
        )
        volume_profile = IBMVolumeProfile(name="10iops-tier", family="tiered")
        ibm_volume = IBMVolume(
            name=transform_ibm_name(self.name), capacity=self.capacity, iops="10iops-tier",
            encryption="provider_managed", volume_index=self.volume_index
        )
    ...
    @classmethod
    def from_softlayer_json(cls, name, volume_json, volume_index):
        response = cls(name=name, capacity=volume_json["diskImage"].get("capacity"), profile="general-profile",
                       iops=3000, delete_volume=True, is_boot_volume=volume_json.get("bootableFlag"),
                       volume_index=volume_index)
        if response.is_boot_volume is True:
            response.volume_index = 0
        return response
...
class SoftLayerNetworkInterface(object):
    ...
    def to_ibm(self):
        ibm_network_interface = IBMNetworkInterface(
            ...
            self.is_primary,
            primary_ipv4_address=self.private_ip,
        ...
        ibm_network_interface.security_groups = security_groups
...
class SoftLayerLoadBalancer(object):
    ...
    def __init__(
            self, name, is_public, host_name, is_data_log_enabled, listeners=None, pools=None, subnets=None
    ):
        self.name = name
        self.is_public = True if is_public == 1 else False
        self.host_name = host_name
        self.is_data_log_enabled = is_data_log_enabled
    ...
    def to_ibm(self):
        ibm_load_balancer = IBMLoadBalancer(
            transform_ibm_name(self.name), is_public=self.is_public, host_name=self.host_name,
            resource_id=None, public_ips=None, private_ips=None, href=None, crn=None, logging_datapath_active=None,
            route_mode=None, operating_status=None, security_groups_supported=None, provisioning_status=None,
            created_at=None
        )
        ibm_load_balancer.host_name = self.host_name
        ibm_load_balancer.logging_datapath_active = self.is_data_log_enabled

        for listener in self.listeners:
            ibm_listener = listener.to_ibm()
            ibm_load_balancer.pools.append(ibm_listener.default_pool)
            ibm_load_balancer.listeners.append(ibm_listener)

        for pool in self.pools:
            ibm_load_balancer.pools.append(pool.to_ibm())
...
class SoftLayerListener(object):
    ...
    def to_ibm(self):
        ibm_listener = IBMListener(
            resource_id=None, port=self.port, protocol=self.protocol.lower(),
            connection_limit=self.connection_limit, href=None
        )
        if self.backend_pool:
            ibm_listener.default_pool = self.backend_pool.to_ibm()
...
class SoftLayerBackendPool(object):
    ...
    def to_ibm(self):
        ...
        for pool_member in self.pool_members:
            ibm_pool.members.append(pool_member.to_ibm())

        if self.health_monitor:
            ibm_pool.health_monitor = self.health_monitor.to_ibm()
...
class SoftLayerPoolMember(object):
    ...
    def __init__(self, weight, port, ip, network_interface=None):
        self.weight = weight
        self.ip = ip
        self.port = port
    ...
    def to_ibm(self):
        ibm_pool_member = IBMPoolMember(self.port, self.weight, target_ip_address=self.ip)

        if not self.network_interface:
            return ibm_pool_member

        ibm_network_interface = self.network_interface.to_ibm()
        ibm_pool_member.subnet_id = ibm_network_interface.ibm_subnet.id
        ibm_pool_member._network_interface = ibm_network_interface
        ibm_pool_member._subnet = ibm_network_interface.ibm_subnet
        return ibm_pool_member
...
class SoftLayerSecurityGroup(object):
    ...
    def to_ibm(self):
        ibm_security_group = IBMSecurityGroup(
            name=transform_ibm_name(self.name), created_at=None, crn=None, href=None, resource_id=None,
...
class SoftLayerSecurityGroupRule(object):
    ...
    def to_ibm(self):
        ...
        ibm_sec_group_rule = IBMSecurityGroupRule(
            ...
            port_max=self.port_min,
            remote_ip_address=self.address if not is_private_ip(self.address) else "0.0.0.0/0",
            remote_cidr_block=self.cidr_block if not is_private_ip(self.cidr_block) else "0.0.0.0/0",
...
class SoftLayerSubnet(object):
    ...
    def to_ibm(self, vyatta=None):
        ibm_subnet = IBMSubnet(
            name=transform_ibm_name(self.name), ipv4_cidr_block=self.network, status=CREATION_PENDING)

        if self.firewalls:
            network_acl = IBMNetworkAcl(name="acl-{}".format(self.name), crn=None, href=None)

            for firewall in self.firewalls:
                if not firewall.list_port_action_protocol_rules():
                    continue

                acl_rule_rule_no = dict()
                for rule in firewall.list_port_action_protocol_rules():
                    ibm_rules = rule.to_ibm(firewall.name)
                    for rule_ in ibm_rules:
                        network_acl.rules.append(rule_)
                        acl_rule_rule_no[network_acl.id] = {"firewall_name": firewall.name,
                                                            "rule_no": rule.rule_no}

                rules_to_keep = list()
                for rule in network_acl.rules.all():
                    protocol = vyatta.get_rule_protocol(**acl_rule_rule_no[network_acl.id])
                    if not protocol:
                        continue
                ...
                network_acl.rules = rules_to_keep
                ...
                if self.list_public_destination_addresses() and len(self.list_public_destination_addresses()):
                    for dest_add in self.list_public_destination_addresses():
                        acl_rule = IBMNetworkAclRule(
                            ...
                            icmp_type=self.type if self.protocol == "icmp" else None,
                            icmp_code=self.code if self.protocol == "icmp" else None,
                            source="0.0.0.0/0",
                            destination=dest_add,
                            tcp_udp_dst_port_min=int(destination_port_min),
                            tcp_udp_dst_port_max=int(destination_port_max),
                            href=None,
                            ip_version="ipv4",
                            resource_id=None,
                        )
                    ...
                    acl_rule = IBMNetworkAclRule(
                        name="{}-rule-{}-{}".format(name, self.rule_no, rule_count),
                        action=action,
                        direction=self.direction,
                        protocol=self.protocol or "all",
                        icmp_type=self.type if self.protocol == "icmp" else None,
                ...
                if self.list_public_destination_addresses() and len(self.list_public_destination_addresses()):
                    for dest_add in self.list_public_destination_addresses():
                        acl_rule = IBMNetworkAclRule(
                            ...
                            tcp_udp_src_port_min=int(source_port_min),
                            tcp_udp_src_port_max=int(source_port_max),
                            href=None,
                            ip_version="ipv4",
                            resource_id=None,
                        )
...
class SoftLayerIpsec(object):
    ...
    def to_ibm(self):
        vpn_gateway = IBMVpnGateway(name=transform_ibm_name(self.name), crn=None, href=None, status=CREATION_PENDING)
        vpn_connection = IBMVpnConnection(
            ...
            peer_cidrs=[tunnel.remote_subnet for tunnel in self.tunnels if tunnel.remote_subnet]
            if self.tunnels else [],
            href=None,
        )
        vpn_connection.ike_policy = (
            self.ike_group.to_ibm() if self.ike_group else None
        )
        vpn_connection.ipsec_policy = (
            self.esp_group.to_ibm() if self.esp_group else None
        )
        vpn_gateway.vpn_connections.append(vpn_connection)
        if self.subnet:
            vpn_gateway.subnet_id = self.subnet.id
        return vpn_gateway
...
class SoftlayerPlacementGroup:
    ...
    def __init__(self, id, name, instances, created_at, strategy="host_spread"):
        self.id = id
        self.name = name
    ...
    @classmethod
    def from_softlayer_json(cls, placement_group_dict):
        return cls(
            id=placement_group_dict["id"],
            name=placement_group_dict["name"],
            created_at=placement_group_dict["createDate"],
            instances=[guest["id"] for guest in placement_group_dict["guests"]]
        )
