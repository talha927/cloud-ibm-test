...
def get_cost_billing_month() -> list:
    """
    Returns the list of previous 12 months date intervals in the following format:
    Example Output: ['2022-06', '2022-07', ...]
    Only returns the current month interval if 'current_month_only' flag is True
    """

    current_month_date = datetime.utcnow().replace(day=1)
    current_month_date_str = current_month_date.strftime("%Y-%m")

    months_list = [current_month_date - relativedelta(months=month_number) for month_number in range(11, 0, -1)]
    months_list_str = [month.strftime("%Y-%m") for month in months_list]
    months_list_str.append(current_month_date_str)

    return months_list_str


def update_cost(cloud_id, m_ibm_cost):
    """
    ...
    m_cost_time = m_ibm_cost.get('last_synced_at')
    cost = IBMCost.from_ibm_json_body(json_body=m_ibm_cost['summary'], cloud_id=cloud_id)
    ...
    with get_db_session() as db_session:
        ...
        update_individual_cost(cloud_id=cloud_id, cost_id=cost_obj.id,
                               m_resources_cost=m_ibm_cost['resources'],
